...
**Rate limits**: `purge()` shares the zone purge API's per-plan rate limits, and a rate-limited call returns `success: false` with a descriptive error. Measured (phase 6): the limit is counted per `purge()` call, so the per-entrypoint fan-out spends one call per entrypoint — four per admin purge with `Record` — and Workers Cache applies the Free-tier limit on every plan; the test suite hit error 1134 after a few dozen calls in quick succession. The admin API surfaces the result object verbatim. Tag purge is available on all plans; only the rate limits vary by plan. Moderation tooling should batch — `tags` accepts arrays, and `tags` + `pathPrefixes` union in a single call.
